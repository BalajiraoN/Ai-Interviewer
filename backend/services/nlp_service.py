...
class NLPService:
    def __init__(self):
        # Allow disabling heavy model loads during development/startup
        disable_heavy = os.getenv("DISABLE_HEAVY_MODELS", "0") == "1"
        self._loaded = ST_AVAILABLE and not disable_heavy
        self.model = None
        if ST_AVAILABLE and not disable_heavy:
            try:
            ...
            if disable_heavy:
                logging.info("DISABLE_HEAVY_MODELS=1 — skipping SentenceTransformer preload.")
            else:
                logging.warning("sentence-transformers not installed. NLP will fallback to length heuristic.")
